...
public class RocksStore implements Store {
...

    {
        ...
        File kvFile =
                new File("/Users/mattison/Projects/openSource/mirror/temp/rocks-db" + UUID.randomUUID(), "mirror-kv");
        try {
            Files.createDirectories(kvFile.getParentFile().toPath());
            Files.createDirectories(kvFile.getAbsoluteFile().toPath());

            File logsFile =
                    new File("/Users/mattison/Projects/openSource/mirror/temp/rocks-db" + UUID.randomUUID(),
                            "mirror-kv");
    ...
    public void addLog(LogEntry logEntry) {
        try {
            ...
            try {
                logs.put(logSize.toString().getBytes(StandardCharsets.UTF_8),
    ...
    public List<LogEntry> subLog(Integer startIndex, Integer endIndex) {
        if (logSize() == 0) {
            ...
            return IntStream.rangeClosed(startIndex, endIndex).mapToObj(i -> {
                try {
